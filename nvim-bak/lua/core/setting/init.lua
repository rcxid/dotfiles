...
-- 设置buffer变量
-- variables: { { key, value }, ... }
function M.set_buffer_variables(variables)
  for _, variable in pairs(variables) do
    local key = variable[1]
    local value = variable[2]
...
-- 设置window变量
-- variables: { { key, value }, ... }
function M.set_window_variables(variables)
  for _, variable in pairs(variables) do
    local key = variable[1]
    local value = variable[2]
...
-- 设置选项卡变量
-- variables: { { key, value }, ... }
function M.set_tabpage_variables(variables)
  for _, variable in pairs(variables) do
    local key = variable[1]
    local value = variable[2]
...
-- 设置预定义的 vim 变量
-- variables: { { key, value }, ... }
function M.set_v_variables(variables)
  for _, variable in pairs(variables) do
    local key = variable[1]
    local value = variable[2]
